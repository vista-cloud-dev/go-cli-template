...
What was found, and the fix status — all ✅ resolved as of 2026-05-27:
...
| 3 | **`settings.local.json` files held one-shot noise** — e.g. a giant `printf` ObjectScript blob, an `awk '/Enterprise Search/…DPTLK7.m'`, `…/tasks/<id>.output` paths, `/tmp/hello` probes. | Clutter; they'll never match again. | ✅ **Fixed** — pruned the one-shot noise from the `vista-cloud-dev` and `m-dev-tools` local files (kept only the reusable rules) and deleted the redundant `go-cli-template` local file; the `~/.claude`, `vista-dev-bridge`, and `vista-iris` local files no longer exist. |
| 4 | **Duplication across repos.** `WebSearch`, `WebFetch(domain:github.com)`, `git add/commit/push` appeared in several separate local files. | You re-grant the same things per repo. | ✅ **Fixed** — removed the duplicate copies from the `vista-cloud-dev` and `m-dev-tools` local files and deleted the `go-cli-template` local file; these rules now live once in `~/.claude/settings.json` (Appendix A). |
